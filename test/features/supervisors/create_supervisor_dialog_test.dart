...
void main() {
  ...
  setUp(() {
    ...
    when(() => repo.list(cursor: any(named: 'cursor')))
        .thenAnswer((_) async => (items: <Supervisor>[], nextCursor: null));
    Get.put(SupervisorsController(repo));
  });
  tearDown(Get.reset);

  Future<void> openDialog(WidgetTester tester) async {
    await tester.pumpWidget(testApp(dialogHost((_) => const CreateSupervisorDialog())));
    await tester.tap(find.text('__open__'));
    await tester.pumpAndSettle();
  }

  Future<void> fill(WidgetTester tester, {required String name, required String email}) async {
  ...
  testWidgets('submits a lowercased email, closes, and shows a snackbar', (tester) async {
    when(() => repo.create(
          name: any(named: 'name'),
          email: any(named: 'email'),
          phone: any(named: 'phone'),
        )).thenAnswer(
      (_) async => const Supervisor(uid: '1', name: 'Ravi', email: 'ravi@x.com', phone: null),
    );
    ...
    verify(() => repo.create(name: 'Ravi', email: 'ravi@x.com', phone: '')).called(1);
    expect(find.byType(CreateSupervisorDialog), findsNothing);
    expect(find.text('Invite sent to ravi@x.com'), findsOneWidget);
  });

  testWidgets('keeps the dialog open and flags the field on a 409', (tester) async {
    when(() => repo.create(
          name: any(named: 'name'),
          email: any(named: 'email'),
          phone: any(named: 'phone'),
